...
def _heads(channels: int, attention_head_dim) -> int:
    """attention_head_dim=None（cat-256）→ 单头；数值 d（butterflies=8）→ channels//d。"""
    return 1 if attention_head_dim is None else channels // attention_head_dim


class Downsample2D(nn.Module):
    """diffusers Downsample2D(use_conv=True)：padding=0 非对称 pad + 3x3 stride2；
    padding=1（butterflies-64）为常规对称填充、无 pad 前置。"""
...
class DownBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, temb_channels, num_layers,
                 ...
                 attention_head_dim=None, downsample_padding=0, eps=1e-6):
        super().__init__()
        ...
        for i in range(num_layers):
            ...
            self.resnets.append(
                ResnetBlock2D(cin, out_channels, temb_channels, groups, eps=eps)
            )
            if with_attention:
                self.attentions.append(
                    AttentionBlock(out_channels, groups, eps=eps,
                                   heads=_heads(out_channels, attention_head_dim))
                )
        self.downsamplers = (
            nn.ModuleList([Downsample2D(out_channels, padding=downsample_padding)])
            if add_downsample
            else None
        )
...
class MidBlock(nn.Module):
    ...
    def __init__(self, channels, temb_channels, groups,
                 attention_head_dim=None, eps=1e-6):
        super().__init__()
        self.resnets = nn.ModuleList([
            ResnetBlock2D(channels, channels, temb_channels, groups, eps=eps),
            ResnetBlock2D(channels, channels, temb_channels, groups, eps=eps),
        ])
        self.attentions = nn.ModuleList([
            AttentionBlock(channels, groups, eps=eps,
                           heads=_heads(channels, attention_head_dim))
        ])
